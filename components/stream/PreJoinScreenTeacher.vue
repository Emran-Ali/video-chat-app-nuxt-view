...
const student = computed(() => lessonDetails.value?.student)
...
const meetingStateTime = ref('--:--')
const canReportNoShow = ref(false)
const showNoShowCountdown = ref(true)
...
const NO_SHOW_TIME_MS = 5 * 60 * 1000 //
...
const tick = () => {
  ...
  const timeSinceScheduledStartSeconds = Math.max(
    0,
    Math.round((now - scheduledTimeMs) / 1000)
  )
  ...
  if (diffSeconds > 0) {
    ...
    meetingStateLabel.value = 'Time since scheduled start'
    meetingStateText.value = 'Start time has passed'
    meetingStateTime.value = formatTime(timeSinceScheduledStartSeconds)
  ...
  >
    ...
    >
      ...
      <p class="text-[11px] text-neutral-400 text-right lg:text-right">
        ...
        <!-- :to="`/teacher/account/report?lessonId=${props.callId}`" -->
        <NuxtLink
          :to="`/report`"
      ...
      <div class="w-full md:flex-1 flex flex-col gap-4 h-full">
        ...
        >
          ...
          >
            ...
            <span
              class="text-md sm:text-xl font-bold tabular-nums text-white"
              >{{ meetingStateTime }}</span
            >
          </div>
        </div>
        <!-- Video + Teacher info -->
        <div class="w-full space-y-6">
          ...
          >
            ...
            <!-- <div v-else class="flex flex-col items-center justify-center">
              <p class="text-base sm:text-lg text-neutral-200">Camera is off</p>
            </div> -->
            ...
            >
              ...
              >
                Let's join the call. The student will be able to join once you
                are on.
              </p>
              <button
      ...
      <div class="w-full lg:w-[340px] flex flex-col gap-4">
        ...
        >
          ...
          >
            <span>{{ meetingStateText }}</span>
            <span
              class="text-xl md:text-2xl font-bold tabular-nums text-white"
              >{{ meetingStateTime }}</span
            >
          </div>
        </div>
        ...
        >
          <p class="text-[0.80rem] uppercase tracking-[0.2em] text-slate-400">
            Set up for a great lesson 🎧
          </p>
          <ul class="space-y-1 text-sm list-disc pl-4">
            <li>Test your microphone and headphones</li>
            <li>Minimize background noise</li>
            <li>
              Choose a clean, well-lit background that feels professional and
              inviting
            </li>
            <li>Tap “Join Lesson” once everything is set.</li>
          </ul>
        </div>
      </div>
    </div>
